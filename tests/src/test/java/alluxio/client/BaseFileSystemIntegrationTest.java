...
import alluxio.client.file.FileInStreamV2;
import alluxio.client.file.FileInStreamV3;
 ...
 */
public final class BaseFileSystemIntegrationTest {
  ...
  /**
   * Tests {@link FileInStreamV2#read()} across block boundary.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#read(byte[])}.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#read(byte[], int, int)} for end of file.
   */
  @Test
  public void readEndOfFile() throws Exception {
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        try (FileInStream is = sFileSystem.openFile(uri,
            FileSystemTestUtils.toOpenFileOptions(op))) {
          byte[] ret = new byte[k / 2];
  ...
  /**
   * Tests {@link FileInStreamV2#seek(long)}. Validate the expected exception for seeking a negative
   * position.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#seek(long)}. Validate the expected exception for seeking a position
   * that is past EOF.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#seek(long)}.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#seek(long)} when at the end of a file at the block boundary.
   */
  @Test
  ...
  /**
   * Tests {@link FileInStreamV2#skip(long)}.
   */
  @Test
  ...
  @Test
  public void forceLoadFile() throws Exception {
    Configuration.set(PropertyKey.USER_FILE_IN_STREAM_VERSION, 2);

    boolean flag4UFS = false;
    ...
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        if (!status.getFileBlockInfos().isEmpty()) {
          FileBlockInfo info = status.getFileBlockInfos().get(0);
          if (info.getBlockInfo().getLocations().isEmpty()) {
            flag4UFS = true;
            checked = true;
          }
        }
        FileInStream is = sFileSystem.openFile(uri, FileSystemTestUtils.toOpenFileOptions(op));
        Assert.assertTrue(is instanceof FileInStreamV2);
        ...
        if (flag4UFS) {
          Assert.assertFalse(
              status.getFileBlockInfos().get(0).getBlockInfo().getLocations().isEmpty());
        }
        flag4UFS = false;
      }
    }
  ...
  //  @Test
  public void forceLoadFileV3() throws Exception {
    ...
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        FileInStream is = sFileSystem.openFile(uri, FileSystemTestUtils.toOpenFileOptions(op));
        Assert.assertTrue(is instanceof FileInStreamV3);
        Assert.assertEquals(k / 2, is.skip(k / 2));
        Assert.assertEquals(k / 2, is.read());
        is.close();
