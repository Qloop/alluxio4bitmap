...
public class AlluxioFileStreamSeekTest {
  ...
  private final static long DEFAULT_BUFFER_SIZE = 1_000_000_00l;
  ...
  public void testWrite() throws Exception {
    ...
    AlluxioURI uri = new AlluxioURI("/benchmark2.txt");
    if (fs.exists(uri)) {
      fs.delete(uri);
    }
...
//        Assert.assertTrue(b[0] == 'a');
      }
//      while (step + is.getPos() < DEFAULT_BUFFER_SIZE) {
//        is.seek(step + is.getPos());
//        count4Seek++;
//        count4Read++;
//      }
  ...
  public void testDisFileReadV2() {
    try {
      ...
      FileSystem fs = FileSystem.Factory.get();
      int size = 1;
      FileInStream[] inputs = new FileInStream[size];

      for (int i = 0; i < size; i++) {
        inputs[i] = fs.openFile(uri, OpenFileOptions.defaults());
      }
      ...
      System.out
          .println("avg time:" + (double) (System.currentTimeMillis() - start) / (count4Read + 1));
      System.out.println("last byte:" + b[0]);
      System.out.println("last byte:" + result);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
