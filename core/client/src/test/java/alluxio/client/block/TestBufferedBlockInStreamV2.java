 ...
 */
public class TestBufferedBlockInStreamV2 extends BufferedBlockInStreamV2 {
  private final byte[] mData;

  /**
   * Constructs a new {@link TestBufferedBlockInStreamV2} to be used in tests.
   *
   * @param blockId the id of the block
   * @param data    data to read
   */
  public TestBufferedBlockInStreamV2(long blockId, byte[] data) {
    super(blockId, data.length);
    mData = data;
    mBuffer.clear();
    mBuffer= ByteBuffer.allocate(data.length);
    mBuffer.put(mData, (int) getPosition(), mBuffer.capacity());
    mBuffer.flip();
  }
