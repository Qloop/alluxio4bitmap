...
public class Input4DoubleV2Test {
  ...
  protected static final int UNIT_SIZE = Double.BYTES;
        ...
        .thenAnswer(new Answer<TestBufferedBlockInStreamV2>() {
          ...
          public TestBufferedBlockInStreamV2 answer(InvocationOnMock invocationOnMock)
            ...
            byte[] input = BufferUtils
                .getIncreasingDoubleByteArray((int) (i * UNIT_LENGTH), (int) UNIT_LENGTH);
  ...
  public void singleReadPos() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((double) i, (double) mTestStream.readDouble(i * UNIT_SIZE), 0);
  ...
  /**
   * Tests that reading through the file one double at a time will yield the correct data.
   */
  @Test
  public void singleRead() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((double) i, (double) mTestStream.readDouble(), 0);
  ...
  public void randomRead() throws Exception {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((double) p, (double) mTestStream.readDouble(p * UNIT_SIZE), 0);
    }
  }
  ...
  public void randomSeek() throws IOException {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((double) p, (double) mTestStream.readDouble(), 0);
    }
  }
  ...
  public void sequenceSkip() throws IOException {
    ...
    double[] con = BufferUtils
        .getIncreasingDoubleArray(0, (int) UNIT_LENGTH);
    for (int i = 0; i < fl / 2; i++) {
      mTestStream.skip((long) UNIT_SIZE);
      v += 1;
      Assert.assertEquals(con[v], (double) mTestStream.readDouble(), 0);
      v += 1;
    }
    Assert.assertEquals(0, mTestStream.remaining());
  ...
  public void randomSkip() throws IOException {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals(con[tPos], (double) mTestStream.readDouble(), 0);
      pos += step + 1;
    }
  }
