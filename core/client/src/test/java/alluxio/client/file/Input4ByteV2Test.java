...
public class Input4ByteV2Test {
  ...
  public void singleReadPos() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((byte) i, (byte) mTestStream.readByte(i * UNIT_SIZE));
  ...
  public void randomRead() throws Exception {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((byte) p, (byte) mTestStream.readByte(p * UNIT_SIZE));
    }
  }
