...
public class Input4FloatV2Test {
  ...
  protected static final int UNIT_SIZE = Float.BYTES;
        ...
        .thenAnswer(new Answer<TestBufferedBlockInStreamV2>() {
          ...
          public TestBufferedBlockInStreamV2 answer(InvocationOnMock invocationOnMock)
            ...
            byte[] input = BufferUtils
                .getIncreasingFloatByteArray((int) (i * UNIT_LENGTH), (int) UNIT_LENGTH);
  ...
  public void singleReadPos() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((float) i, (float) mTestStream.readFloat(i * UNIT_SIZE), 0);
  ...
  /**
   * Tests that reading through the file one float at a time will yield the correct data.
   */
  @Test
  public void singleRead() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((float) i, (float) mTestStream.readFloat(), 0);
  ...
  public void randomRead() throws Exception {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((float) p, (float) mTestStream.readFloat(p * UNIT_SIZE), 0);
    }
  }
  ...
  public void randomSeek() throws IOException {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((float) p, (float) mTestStream.readFloat(), 0);
    }
  }
  ...
  public void sequenceSkip() throws IOException {
    ...
    long fl = UNIT_LENGTH;
    int v = 0;
    float[] con = BufferUtils
        .getIncreasingFloatArray(0, (int) UNIT_LENGTH);
    for (int i = 0; i < fl / 2; i++) {
      mTestStream.skip((long) UNIT_SIZE);
      v += 1;
      Assert.assertEquals(con[v], (float) mTestStream.readFloat(), 0);
      v += 1;
    }
    Assert.assertEquals(0, mTestStream.remaining());
  ...
  public void randomSkip() throws IOException {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals(con[tPos], (float) mTestStream.readFloat(), 0);
      pos += step + 1;
    }
  }
